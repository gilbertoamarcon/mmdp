...
do
  for g in {1..4}
  do
    name="$prob"_"$x"x"$y"_g"$g"_i"$i"
    mkdir sim/$name
    mkdir sim/$name/fig
    python src/generate-problem.py -e 0.1 -x $x -y $y -d 0.$i -g $g -o problems/$name.yaml
    ...
    for k in {1..10}
    do
      mkdir sim/$name/"$iter"h_"$k"
      python src/simulate.py -i problems/$name.yaml -s sols/$name.yaml -o sim/$name/"$iter"h_"$k" -x $iter -f ../fig/state_%09d.svg
    done
  done
done
